...
engine = create_engine(SQLALCHEMY_DATABASE_URL, connect_args = {"check_same_tread" : False}) #fastapi works with multiple thread. Allows FastAPI (multi-threaded) to use the same SQLite connection safely
